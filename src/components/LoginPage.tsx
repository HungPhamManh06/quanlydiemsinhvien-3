              ...
              <div className="p-8 sm:p-10">
                ...
                <div className="mt-6 p-4 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl border border-blue-100">
                  ...
                  <div className="space-y-2">
                    ...
                    <button
                      type="button"
                      onClick={() => { setLoginUsername('Giangvien'); setLoginPassword('Gv@123456'); }}
                      className="w-full flex items-center justify-between p-2.5 bg-white rounded-lg border border-blue-200 hover:border-blue-400 hover:shadow-sm transition-all text-left group"
                    >
                      <div>
              ...
              <div className="p-8 sm:p-10">
                ...
                <form onSubmit={handleRegister} className="space-y-4">
                    ...
                    {regPassword.length > 0 && (
                      <div className="mt-2.5 p-3 bg-gray-50 rounded-xl border border-gray-200 space-y-1.5">
                        <p className="text-[11px] font-semibold text-gray-600 mb-2 flex items-center gap-1">
                          <Shield className="w-3 h-3" /> Yêu cầu mật khẩu:
                        </p>
                        <div className={`flex items-center gap-2 text-[11px] font-medium ${pwdChecks.minLength ? 'text-emerald-600' : 'text-red-400'}`}>
                          {pwdChecks.minLength ? <CheckCircle2 className="w-3.5 h-3.5 shrink-0" /> : <XCircle className="w-3.5 h-3.5 shrink-0" />}
                          Ít nhất 6 ký tự
                        </div>
                        <div className={`flex items-center gap-2 text-[11px] font-medium ${pwdChecks.hasUppercase ? 'text-emerald-600' : 'text-red-400'}`}>
                          {pwdChecks.hasUppercase ? <CheckCircle2 className="w-3.5 h-3.5 shrink-0" /> : <XCircle className="w-3.5 h-3.5 shrink-0" />}
                          Ít nhất 1 chữ cái viết hoa (A-Z)
                        </div>
                        <div className={`flex items-center gap-2 text-[11px] font-medium ${pwdChecks.hasNumber ? 'text-emerald-600' : 'text-red-400'}`}>
                          {pwdChecks.hasNumber ? <CheckCircle2 className="w-3.5 h-3.5 shrink-0" /> : <XCircle className="w-3.5 h-3.5 shrink-0" />}
                          Ít nhất 1 chữ số (0-9)
                        </div>
                        <div className={`flex items-center gap-2 text-[11px] font-medium ${pwdChecks.hasSpecial ? 'text-emerald-600' : 'text-red-400'}`}>
                          {pwdChecks.hasSpecial ? <CheckCircle2 className="w-3.5 h-3.5 shrink-0" /> : <XCircle className="w-3.5 h-3.5 shrink-0" />}
                        ...
                        <div className="mt-2">
                          <div className="flex gap-1">
                            {[0,1,2,3].map(i => {
                              const strength = [pwdChecks.minLength, pwdChecks.hasUppercase, pwdChecks.hasNumber, pwdChecks.hasSpecial].filter(Boolean).length;
                              return (
                          ...
                          <p className={`text-[10px] font-semibold mt-1 ${
                            [pwdChecks.minLength, pwdChecks.hasUppercase, pwdChecks.hasNumber, pwdChecks.hasSpecial].filter(Boolean).length === 4
                              ? 'text-emerald-600' : 'text-orange-500'
                          }`}>
                            {(() => {
                              const s = [pwdChecks.minLength, pwdChecks.hasUppercase, pwdChecks.hasNumber, pwdChecks.hasSpecial].filter(Boolean).length;
                              if (s === 0) return '';
                  ...
                  <div>
                    ...
                    {regConfirmPassword.length > 0 && (
                      <p className={`text-[11px] font-medium mt-1.5 flex items-center gap-1 ${regConfirmPassword === regPassword ? 'text-emerald-600' : 'text-red-500'}`}>
                        {regConfirmPassword === regPassword
