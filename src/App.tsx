  ...
  return (
    <ToastProvider>
      <div className={`min-h-screen bg-gray-50 flex ${darkMode ? 'dark' : ''}`}>
        ...
        {/* Sidebar */}
        <aside className={`fixed lg:sticky top-0 left-0 z-50 h-screen w-[260px] bg-white border-r border-gray-100 shadow-lg lg:shadow-none flex-col transition-transform duration-300 hidden lg:flex ${sidebarOpen ? 'translate-x-0 !flex' : '-translate-x-full lg:translate-x-0'
          } print:hidden`}>
          {/* Logo */}
          ...
          <nav className="flex-1 p-3 space-y-0.5 overflow-y-auto">
            ...
            {tabs.map(tab => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all relative ${activeTab === tab.id
          ...
          <div className="grid grid-cols-6 items-center">
            {tabs.map(tab => (
              <button
                key={tab.id}
                onClick={() => handleTabChange(tab.id)}
                className={`flex flex-col items-center justify-center py-2 px-1 transition-all ${activeTab === tab.id
                    ? 'text-blue-600'
                    : 'text-gray-400 hover:text-gray-600'
                  }`}
              >
                <div className={`p-1.5 rounded-xl transition-all ${activeTab === tab.id
                    ? 'bg-blue-50 scale-110'
                    : ''
                  }`}>
                  {tab.icon}
                </div>
                <span className={`text-[9px] font-medium mt-0.5 leading-tight text-center ${activeTab === tab.id ? 'text-blue-600' : 'text-gray-400'
                  }`}>
                  {tab.label}
                </span>
                {activeTab === tab.id && (
